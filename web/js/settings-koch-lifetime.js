// Settings page — Koch lifetime view modal.
//
// Opens from the "Full history →" link in the rollup heading. Shows
// the complete per-band history for the current claimed_set_key plus
// a chronological list of gear-change events. This is the engine-side
// monitoring surface for "how have the bands and gears shifted over
// time"; nothing here is shown on the listening surface.

const dialog = document.getElementById("settings-koch-lifetime-dialog");
const titleEl = document.getElementById("settings-koch-lifetime-title");
const body = document.getElementById("settings-koch-lifetime-body");
const link = document.getElementById("settings-koch-history-link");
...
function buildGearChangesSection(events) {
    ...
    // Latest first reads more naturally for a "what just happened" list.
...
function buildHistoryGridSection(history) {
    ...
    if (sessions.length === 0 || bands.length === 0) {
        const empty = document.createElement("p");
        empty.className = "settings-koch-lifetime__empty";
        empty.textContent = "No saved-answer history yet.";
        section.appendChild(empty);
        return section;
    }
    ...
    // Index entries by run_index for fast lookup when a band did not
    // appear in a session (defensive — should not happen with the
    // generator today, but legacy records could be missing a slot).
    bands.forEach((band) => {
...
async function loadHistory(claimedSetKey) {
    const url = claimedSetKey
        ? `/api/koch-band-history?claimed_set_key=${encodeURIComponent(claimedSetKey)}`
        : "/api/koch-band-history";
