// Settings -> Recognition tab: attention response summary.
//
// Uses saved per-exercise S/T evidence to show whether signal texture
// helped or hurt recognition accuracy, misses, and confusions.

const root = document.getElementById("settings-recognition-attention");
const meta = document.getElementById("settings-recognition-attention-meta");
const tbody = document.getElementById("settings-recognition-attention-tbody");

const AXIS_ORDER = ["accuracy", "misses", "confusions", "overall"];
const AXIS_LABELS = {
    accuracy: "Accuracy",
    misses: "Miss avoidance",
    confusions: "Confusion avoidance",
    overall: "Overall",
};

const MAIN_HEADER_TOOLTIPS = {
    Condition: "Signal-condition grouping from saved recognition exercises.",
    Overall: "Combined recognition response compared with the opposite S condition.",
    "S/T": "Observed RST Strength and Tone values.",
    Details: "Open per-axis values, deltas, and evidence.",
};

const DETAIL_HEADER_TOOLTIPS = {
    Axis: "Recognition axis being compared.",
...
function renderProfile(profile) {
    ...
    if (!conditions.length || Number(profile?.exercise_count || 0) <= 0) {
        ...
        meta.textContent = `${claimed} · no S/T-tagged recognition evidence yet`;
        root.hidden = false;
        return;
    }
...
async function loadAttentionResponse({ force = false } = {}) {
    if (!root || !meta || !tbody || isLoading) return;
    if (hasLoaded && !force) return;
    isLoading = true;
    try {
        const res = await fetch("/api/recognition-attention-response", { cache: "no-store" });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        renderProfile(await res.json());
        hasLoaded = true;
    } catch {
        tbody.replaceChildren();
        meta.textContent = "Attention response could not be loaded.";
        root.hidden = false;
    } finally {
        isLoading = false;
    }
...
window.addEventListener("copy-settings-records-changed", (event) => {
    if (event.detail?.kind === "recognition" && hasLoaded) {
        loadAttentionResponse({ force: true });
    }
});
