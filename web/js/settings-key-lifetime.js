// Settings page — Key (cadence) lifetime view modal.
//
// Mirrors settings-koch-lifetime.js for the Key tab. Opens from the
// "Full history →" link in the Key rollup heading and shows the
// complete per-band history for the current cadence claimed_set_key
// plus a chronological list of gear-change events. Engine-side
// diagnostic surface only — nothing here is shown during a send
// session.

const dialog = document.getElementById("settings-key-lifetime-dialog");
const titleEl = document.getElementById("settings-key-lifetime-title");
const body = document.getElementById("settings-key-lifetime-body");
const link = document.getElementById("settings-key-history-link");
...
function buildHistoryGridSection(history) {
    ...
    if (sessions.length === 0 || bands.length === 0) {
        const empty = document.createElement("p");
        empty.className = "settings-koch-lifetime__empty";
        empty.textContent = "No saved send history yet.";
        section.appendChild(empty);
        return section;
    }
...
async function loadHistory(claimedSetKey) {
    const url = claimedSetKey
        ? `/api/cadence-band-history?claimed_set_key=${encodeURIComponent(claimedSetKey)}`
        : "/api/cadence-band-history";
