...
class KeyElement:
    """One already-formed key element at its start time in seconds."""
...
class KeyDecoder:
    ...
    def __init__(self, *, dit_seconds: float, character_gap_dits: int = 3) -> None:
        if dit_seconds <= 0:
        ...
        self._character_gap_seconds = dit_seconds * character_gap_dits
    ...
    def push(self, element: KeyElement) -> DecodedSymbol | None:
        ...
        if self._last_element_end is not None:
            gap_seconds = element.timestamp - self._last_element_end
            if gap_seconds < 0:
        ...
        if self._started_at is None:
            self._started_at = element.timestamp
        self._marks.append(mark)
        self._last_element_end = element.timestamp + self._duration_for(element.kind)
        return decoded
    ...
    def _duration_for(self, kind: ElementKind) -> float:
        return self._dit_seconds if kind == "dit" else self._dit_seconds * 3
