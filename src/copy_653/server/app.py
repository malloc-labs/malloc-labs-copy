...
from copy_653.audio import patterns, playback, synth, timing
...
WORD_DETECTION_INSTRUCTION_SYMBOLS: frozenset[str] = frozenset({"K", "M", "U"})
...
def _synthesize_word_detection_audio(
    words: list[str],
    focus_symbols: tuple[str, ...],
    audio_params: AudioParameters,
) -> tuple[np.ndarray, list[tuple[str, float, float, int, str]]]:
    """Render Word Detection audio, including spoken per-word focus prompts.

    Prompts are intentionally limited to the early Koch focus symbols with
    shipped instruction recordings: K, M, and U. Other focus symbols keep the
    original Morse-only word block.
    """
    focus_instruction_symbols = WORD_DETECTION_INSTRUCTION_SYMBOLS & set(focus_symbols)
    ...
    timeline: list[tuple[str, float, float, int, str]] = []
    cursor = 0.0
    ...
    for word_index, word in enumerate(words, start=1):
        ...
        instruction = _word_detection_instruction_samples(
            word,
...
def _word_detection_instruction_samples(
    word: str,
    ...
    symbols = _word_detection_instruction_symbols(word, focus_instruction_symbols)
    if not symbols:
...
def _word_detection_instruction_symbols(
...
async def _start_word_detection_action(
    ...
    samples, timeline = _synthesize_word_detection_audio(
        word_list,
        generated.focus_set,
        audio_params,
    )
